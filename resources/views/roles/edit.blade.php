...
<div class="container-fluid p-4">
    ...
    <div class="page-header">
        <h1 class="page-title">Edit Role: {{ $role->role_name }}</h1>
        <div>
    ...
    <!-- Form -->
    <form action="{{ route('roles.update', $role) }}" method="POST" id="updateForm">
        @csrf
        @method('PUT')
                            ...
                            <div class="col-md-6">
                                <div class="mb-4">
                                    ...
                                    <input type="text" name="role_name" id="role_name" class="form-control" 
                                           placeholder="Enter role name" value="{{ old('role_name', $role->role_name) }}" required>
                                    @error('role_name')
                            ...
                            <div class="col-md-6">
                                <div class="mb-4">
                                    ...
                                    <input type="text" name="description" id="description" class="form-control" 
                                           placeholder="Enter role description" value="{{ old('description', $role->description) }}">
                                    @error('description')
                                        <div class="text-danger small mt-1">{{ $message }}</div>
                                    @enderror
                                </div>
                                                        ...
                                                        <div class="permission-checkbox-item">
                                                            <div class="form-check">
                                                                <input type="checkbox"
                                                                    ...
                                                                    id="{{ $id }}"
                                                                    {{ in_array($permission->access_id, $currentPermissions) ? 'checked' : '' }}
...
<!-- Hidden Delete Form -->
<form action="{{ route('roles.destroy', $role) }}" method="POST" id="deleteForm" style="display: none;">
    @csrf
    @method('DELETE')
</form>
@endsection
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Pre-select quick select button based on current permissions
        const checkboxes = document.querySelectorAll('.permission-checkbox:checked');
        if (checkboxes.length > 0) {
            const firstChecked = checkboxes[0];
            const label = firstChecked.closest('.form-check').querySelector('.form-check-label').textContent.trim();
            const accessLevel = getAccessLevelFromLabel(label);
            
            // Check if all checkboxes have the same access level
            let allSameLevel = true;
            checkboxes.forEach(checkbox => {
                const checkLabel = checkbox.closest('.form-check').querySelector('.form-check-label').textContent.trim();
                const checkAccessLevel = getAccessLevelFromLabel(checkLabel);
            ...
            if (allSameLevel) {
                const quickSelectBtn = document.querySelector(`.quick-select-btn[onclick*="${accessLevel}"]`);
                if (quickSelectBtn) {
                    quickSelectBtn.classList.add('active');
                }
            }
        }
    });
    
    // Form validation - COMMENTED OUT TEMPORARILY FOR TESTING
    document.querySelector('#updateForm').addEventListener('submit', function(e) {
        ...
        if (checkedPermissions.length === 0) {
            e.preventDefault();
            if (confirm('This role will have no permissions. Are you sure you want to update it?')) {
                return true;
            }
            return false;
        }
