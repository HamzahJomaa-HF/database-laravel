...
@section('title', 'Portfolios Management - Edit Portfolio')
...
<div class="dashboard-content">
    <div class="form-container">
        ...
        <div class="delete-form-container">
            ...
            <!-- DELETE FORM - MOVED OUTSIDE EDIT FORM -->
        ...
        <!-- EDIT FORM - DELETE FORM REMOVED FROM HERE -->
        <form id="editPortfolioForm" action="{{ route('portfolios.update', $portfolio->portfolio_id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="card">
                            ...
                            <div class="col-12">
                                <div class="mb-3">
                                    ...
                                    <textarea name="description" 
                                              ...
                                              rows="3">{{ old('description', $portfolio->description) }}</textarea>
                                    @error('description')
                                        <div class="error-message">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-12">
                                <div class="mb-3">
                                    ...
                                    <select name="cops[]" id="cops" class="form-control select2" multiple="multiple">
                                        @php
                                            // Use old input if exists, otherwise use currentCopIds from controller
                                            $selectedCopIds = old('cops', $currentCopIds ?? []);
                                        @endphp
                                        
                                        @foreach($cops as $cop)
                                            <option value="{{ $cop->cop_id }}" 
                                                {{ in_array($cop->cop_id, $selectedCopIds) ? 'selected' : '' }}>
                                                {{ $cop->cop_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <div class="projects-info">
                                        <i class="fas fa-info-circle"></i> Select multiple COPs or leave empty for none
                                    </div>
                                    @error('cops')
                                        <div class="error-message">{{ $message }}</div>
                                    @enderror
                ...
                <div class="card-footer">
                    <div class="buttons-wrapper">
                        <a href="{{ route('portfolios.index', $portfolio->portfolio_id) }}" class="btn btn-outline-primary">
                            Cancel
                        </a>
