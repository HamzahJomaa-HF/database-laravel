    ...
    <div class="main-div">
        ...
        <div class="pagination-container">
            ...
            <div class="pagination-numbers">
                ...
                @else
                    <a href="{{ $cops->appends(request()->query())->url(1) }}" class="pagination-nav-button" title="First Page">
                        <i class="fas fa-angle-double-left"></i>
                    </a>
                @endif
                ...
                @else
                    <a href="{{ $cops->appends(request()->query())->previousPageUrl() }}" class="pagination-nav-button" title="Previous Page">
                        <i class="fas fa-chevron-left"></i>
                    </a>
                @endif
                ...
                @php
                    $currentPage = $cops->currentPage();
                    $lastPage = $cops->lastPage();
                    $start = max(1, $currentPage - 2);
                    $end = min($lastPage, $currentPage + 2);

                    if ($start > 1) {
                        echo '<a href="' . $cops->appends(request()->query())->url(1) . '" class="pagination-button">1</a>';
                        if ($start > 2) {
                    ...
                    for ($i = $start; $i <= $end; $i++) {
                        if ($i == $currentPage) {
                            ...
                            echo '<a href="' . $cops->appends(request()->query())->url($i) . '" class="pagination-button">' . $i . '</a>';
                        }
                    }
                    ...
                    if ($end < $lastPage) {
                        ...
                        echo '<a href="' . $cops->appends(request()->query())->url($lastPage) . '" class="pagination-button">' . $lastPage . '</a>';
                    }
                @endphp

                {{-- Next Page --}}
                @if($cops->hasMorePages())
                    <a href="{{ $cops->appends(request()->query())->nextPageUrl() }}" class="pagination-nav-button" title="Next Page">
                ...
                {{-- Last Page --}}
                @if($cops->hasMorePages())
                    <a href="{{ $cops->appends(request()->query())->url($cops->lastPage()) }}" class="pagination-nav-button" title="Last Page">
