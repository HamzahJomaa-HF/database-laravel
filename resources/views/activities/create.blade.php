        ...
        <div class="col-12">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4 pt-3">
                <div>
                    ...
                    <h1 class="h2 fw-bold mb-1">Create New Activity</h1>
                    <p class="text-muted mb-0">Add new activity information and details</p>
                </div>
        ...
        <div class="col-12">
            <div class="card shadow-sm border-0">
                ...
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('activities.store') }}" method="POST" class="needs-validation" novalidate id="activityForm">
                        @csrf
                                        ...
                                        <select id="action_plan_id" 
                                            ...
                                            @foreach($actionPlans as $actionPlan)
                                                <option value="{{ $actionPlan->action_plan_id }}" 
                                                        {{ old('action_plan_id') == $actionPlan->action_plan_id ? 'selected' : '' }}>
                                                    {{ $actionPlan->title }}
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success && response.projects && response.projects.length > 0) {
                        ...
                        // Get old project selections
                        const oldProjects = {!! json_encode(old('projects', [])) !!};
                            ...
                            if (groupedProjects[groupKey] && groupedProjects[groupKey].length > 0) {
                                ...
                                groupedProjects[groupKey].forEach(project => {
                                    programGroup.append($('<option>')
                                        ...
                                        .prop('selected', oldProjects.includes(project.project_id.toString()))
                                        .text(project.name)
                                    );
                                });
        ...
        // Load projects if program is already selected from old input
        const initialProgramId = $('#programs_select').val();
        if (initialProgramId) {
        ...
        // ============================================
        // Get old values for form repopulation
        const oldComponentId = '{{ old("rp_component_id", "") }}';
        const oldRpActivities = {!! json_encode(old('rp_activities', [])) !!};
        
        // Initialize all selects
        $(document).ready(function() {
            // Initialize Action Plan Select2
            const actionPlanSelect = $('#action_plan_id');
            const oldActionPlanId = actionPlanSelect.val();
            ...
            // Ensure old value is preserved
            if (oldActionPlanId) {
                actionPlanSelect.val(oldActionPlanId).trigger('change');
            }
            ...
            if (componentSelect.find('option').length > 1) {
                ...
                // Ensure old value is preserved
                if (oldComponentId) {
                    componentSelect.val(oldComponentId).trigger('change');
                }
            }
            ...
            // Load activities if component is already selected from old input
            if (oldComponentId && oldRpActivities.length > 0) {
                setTimeout(function() {
                    loadRPActivitiesByComponent(oldComponentId);
                }, 300);
            }
        });
                        ...
                        success: function(response) {
                            ...
                            if (response.success && response.data && response.data.length > 0) {
                                ...
                                response.data.forEach(component => {
                                    ...
                                    // Select if it was in old input
                                    if (oldComponentId && component.rp_components_id == oldComponentId) {
                                ...
                                // Ensure the old value is set in Select2
                                if (foundSelected && oldComponentId) {
                                    componentsSelect.val(oldComponentId).trigger('change');
                                }
                            } else {
                ...
                success: function(response) {
                    ...
                    if (response.success && response.data && Array.isArray(response.data)) {
                        if (response.data.length === 0) {
                            ...
                            // Select previously chosen activities from old input
                            if (oldRpActivities && oldRpActivities.length > 0) {
                                const stringIds = oldRpActivities.map(id => String(id));
        ...
        // Load components on page load if action plan is already selected
        $(document).ready(function() {
            const initialActionPlanId = $('#action_plan_id').val();
            
            // If component is already selected from old input, load its activities
            if (oldComponentId && oldRpActivities.length > 0) {
                setTimeout(function() {
                    loadRPActivitiesByComponent(oldComponentId);
                }, 300);
        ...
        function handleVenueChange() {
            ...
            if (selectedValue === 'Other') {
                ...
                // Update hidden field with custom venue value
                if (customVenueInput.val().trim()) {
