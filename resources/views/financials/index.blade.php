...
@section('styles')
    ...
    <style>
        ...
        .bg-omt {
            background-color: rgb(255, 252, 48) !important;
            color: black;
        }
        ...
        /* Pagination Styles - Organized Footer */
        ...
        /* View Details button - matches amount color */
        .btn-details {
            background-color: var(--primary-color);
            color: white;
            border: none;
            ...
            @if($financials->count() > 0)
                <table class="table">
                    ...
                    <tbody>
                        ...
                        @php
                            // Get medical subtype for display
                            $medicalSubtype = $financial->financial_type === 'medical' ? ($financial->financial_data['medication_type'] ?? null) : null;
                        @endphp
                        <tr data-id="{{ $financial->activity_financial_id }}" 
                            ...
                            data-medical-subtype="{{ $medicalSubtype }}">
                            <td>
                            ...
                            <td>
                                @if($financial->financial_type === 'medical' && $medicalSubtype)
                                    <span class="badge bg-{{ $medicalSubtype === 'medicine' ? 'medicine' : 'hospital' }}">
                                        {{ strtoupper($financial->financial_type) }} - {{ ucfirst($medicalSubtype) }}
                                    </span>
                                @else
                                    <span class="badge bg-{{ $financial->financial_type }}">
                                        {{ strtoupper($financial->financial_type) }}
                                    </span>
                                @endif
                            </td>
                            <td>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="fas fa-info-circle me-2"></i>Financial Details
                </h5>
...
<script>
    ...
    let currentFinancialType = null;
    let currentMedicalSubtype = null;
    ...
    function showDetails(financialId) {
        ...
        const typeBadge = row.querySelector('.badge.bg-omt, .badge.bg-medical, .badge.bg-education, .badge.bg-medicine, .badge.bg-hospital');
        let typeText = typeBadge?.innerText.trim().toLowerCase() || 'omt';
        
        if (typeText.includes('medical')) {
            currentFinancialType = 'medical';
            currentMedicalSubtype = row.getAttribute('data-medical-subtype');
        } else if (typeText.includes('omt')) {
            currentFinancialType = 'omt';
            currentMedicalSubtype = null;
        } else {
            currentFinancialType = 'education';
            currentMedicalSubtype = null;
        }
        ...
        const txDate = row.getAttribute('data-tx-date');
    ...
    function renderViewMode() {
        ...
        let displayType = currentFinancialType.toUpperCase();
        if (currentFinancialType === 'medical' && currentMedicalSubtype) {
            displayType = `MEDICAL - ${currentMedicalSubtype.toUpperCase()}`;
        }
        
        let html = `
            <div class="details-section">
                <div class="details-section-title">
                    <i class="fas fa-chart-line me-2"></i>${displayType} Financial Details
                </div>
        ...
        // Display all financial_data fields
        for (const [key, value] of Object.entries(currentFinancialData)) {
            if (value !== null && value !== '' && value !== undefined) {
                const formattedLabel = formatLabel(key);
                const formattedValue = formatValue(value, key);
                const isHighlight = key.includes('percentage') || key.includes('scholarship') || key.includes('reciver') || key.includes('receiver') || key.includes('medical_category') || key.includes('medication_type');
                html += `
    ...
    function enableEditMode() {
        ...
        let html = `
            <div class="details-section">
                <div class="details-section-title">
                    <i class="fas fa-edit me-2"></i>Edit ${displayType} Financial Details
                </div>
                <div class="details-grid">
                    <div class="details-item">
                        <span class="details-label">Amount:</span>
    ...
    function saveEditMode() {
        ...
        // Ensure medication_type is preserved for medical records
        if (currentFinancialType === 'medical' && currentMedicalSubtype && !updatedFinancialData.medication_type) {
            updatedFinancialData.medication_type = currentMedicalSubtype;
        }
        ...
        .then(data => {
            if (data.success) {
                ...
                if (row) {
                    ...
                    // Update medical subtype badge if applicable
                    if (currentFinancialType === 'medical' && updatedFinancialData.medication_type) {
                        const typeBadge = row.querySelector('.badge.bg-medicine, .badge.bg-hospital, .badge.bg-medical');
                        if (typeBadge) {
                            const newSubtype = updatedFinancialData.medication_type;
                            if (newSubtype === 'medicine') {
                                typeBadge.className = 'badge bg-medicine';
                                typeBadge.textContent = `MEDICAL - ${newSubtype.toUpperCase()}`;
                            } else if (newSubtype === 'hospital') {
                                typeBadge.className = 'badge bg-hospital';
                                typeBadge.textContent = `MEDICAL - ${newSubtype.toUpperCase()}`;
                            }
                            row.setAttribute('data-medical-subtype', newSubtype);
                            currentMedicalSubtype = newSubtype;
                        }
                    }
                }
                ...
                // Show success toast
                showToast('success', 'Financial record updated successfully!');
            } else {
        ...
        .catch(error => {
            ...
            showToast('error', 'Failed to update financial record');
        });
    }
